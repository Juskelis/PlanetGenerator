...
namespace LibNoise.Unity
{
    ...
    public class Noise2D : IDisposable
    {
        ...
        public void GenerateSpherical(double south, double north, double west, double east)
        {
            if (east <= west)
            {
                throw new ArgumentException("east needs to be greater than west");
            }
            if (north <= south)
            {
                throw new ArgumentException("north needs to be greater than south");
            }
            if (this.m_generator == null)
            {
                throw new ArgumentException("generator needs to exist");
            }
